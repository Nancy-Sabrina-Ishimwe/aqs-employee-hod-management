...
import projectData from "../Service/DataServices";

const handleAddProject = ({ sendproject }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await projectData.addProject(project);
      sendproject(project);
      setProjectName("");
      setDescription("");
      setProjectcategory("");
...
const Schedule = () => {
  ...
  const handleAddTask = () => {
    if (newTask.trim()) {
      setTasks([
        ...tasks,
        { id: tasks.length + 1, task: newTask, assignedTo: "" },
      ]);
  ...
  const handleArrowClick = (direction) => {
    if (direction === "left") {
      ...
      setCurrentRangeStart((prev) =>
        Math.min(prev + 1, timelineDates.length - 4)
      );
    }
  };
          ...
          <div className="col-span-2 grid grid-cols-2 gap-6">
            <div className="p-6 rounded-md">
              ...
              <div className="mt-8">
                ...
                <div className="flex items-center justify-between mb-4 start_date">
                  <button
                ...
                <div className="flex space-x-2 mb-4 end_date">
                  {latestProjectDates.map((date, index) => (
                    <button
                      key={index}
                      className={`px-4 py-2 rounded-full text-center ${
                        expandedLatestDates[index]
                          ? "bg-[#264667] text-white"
                          : "bg-gray-200 text-gray-600"
                      }`}
            ...
            <div className="bg-white p-6 rounded-md shadow-md">
              ...
              <div className="grid grid-cols-7 gap-2 mb-4">
                {Array.from({ length: 30 }).map((_, index) => (
                  <button
                    ...
                    onClick={() =>
                      handleDateChange(new Date(2024, 9, index + 1))
                    }
                  >
                    {index + 1}
                  </button>
              ...
              <button className="px-4 py-2 bg-[#264667] text-white rounded-md">
                Add to schedule
              </button>
            </div>
            <div>
              <button className="bg-[#264667] text-white px-4 py-2 rounded-md ">
                Save
              </button>
            </div>
          </div>
      ...
      <form>
        <div className="grid grid-cols-1 mt-8">
          <div className="col-span-1 bg-white p-6 rounded-md shadow-md space-y-4">
              ...
              >
                ...
                <div className="flex items-center space-x-2">
                  <select
                    ...
                    onChange={(e) =>
                      handleAssignToChange(task.id, e.target.value)
                    }
                  >
            ...
            <button className="px-4 py-2 w-60 items-center bg-[#264667] text-white rounded-md mt-4 ml-80">
              Send
            </button>
          </div>
        </div>
